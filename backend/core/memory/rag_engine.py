...
from .evolution_replayer import TacticEvolutionReplayer
...
from .tactic_catalog import ALL_TACTIC_SEEDS, TACTIC_NAME_BY_ID, get_seeds_for_sport
from .tactic_graph import TacticGraph
...
class RAGEngine:
    def __init__(self):
        ...
        self.collection = self.client.get_or_create_collection(
            name="sports_tactics_bayesian",
            embedding_function=self.emb_fn,
        )
        self.optimizer = TacticOptimizer()
        self.scheduler = PolicyScheduler()
        self.reranker = RetrievalReranker()
        self.evolution_replayer = TacticEvolutionReplayer()
        self.scenario_memory = ScenarioMemory(os.path.join(DB_PATH, "scenario_memory.json"))
        self.tactic_graph = TacticGraph(ALL_TACTIC_SEEDS)
        self._sync_seed_tactics("badminton")
        self._sync_seed_tactics("table_tennis")

    def _sync_seed_tactics(self, sport_type: str = "badminton"):
        seeds = get_seeds_for_sport(sport_type)
        print(f"Syncing Bayesian knowledge base for {sport_type} ({len(seeds)} seeds)...")
    ...
    def retrieve(self, query_text, context=None, n_results=3, sport_type="badminton"):
        where_filter = {"sport": sport_type}
        results = self.collection.query(query_texts=[query_text], n_results=12, where=where_filter)
        ...
        for index, document in enumerate(documents):
            ...
            graph_profile = self.tactic_graph.related_profile(tactic_id, context)
            graph_bias = float(graph_profile.get("graph_bias", 0.0) or 0.0)
            schedule = self.scheduler.schedule_retrieval(context, scenario_summary, metadata)
            scheduler_bias = float(schedule.get("scheduler_bias", 0.0) or 0.0)
            temperature = float(schedule.get("exploration_temperature", 1.0) or 1.0)
            final_score = float(np.clip(optimization["final_score"] * temperature + scenario_bias + scheduler_bias + graph_bias, 0.0, 1.42))
            ...
            candidates.append(
                {
                    ...
                    "related_tactics": graph_profile.get("related_tactics", []),
                    "transition_family": graph_profile.get("transition_family", "isolated"),
                    ...
                    "debug_stats": (
                        ...
                        f"scenario={scenario_bias:.2f}/scheduler={scheduler_bias:.2f}/graph={graph_bias:.2f}"
                    ),
                }
            )

        candidates.sort(key=lambda item: item["score"], reverse=True)
        reranked = self.reranker.rerank(candidates, context=context, scenario_summary=scenario_summary)
        replayed = self.evolution_replayer.build_candidate_replays(reranked, context=context, scenario_summary=scenario_summary)
        return replayed[:n_results]

    def update_policy(self, tactic_id, reward, context=None, sport_type="badminton"):
        result = self.collection.get(ids=[tactic_id], where={"sport": sport_type})
        ...
        learning_scale = float(schedule.get("learning_rate_scale", 1.0) or 1.0)
        if reward >= 0:
        ...
        evolution_summary = self.evolution_replayer.summarize_update(
            tactic_id=tactic_id,
            reward=reward,
            update_payload={
                **plan,
            ...
            context=context,
            tactic_name=current_meta.get("name") or TACTIC_NAME_BY_ID.get(tactic_id) or tactic_id,
        )
