...
def simulate_rally(video_path, mode='singles', sport_type='badminton'):
    """
    ...
    try:
        with open(video_path, 'rb') as file_obj:
            files = {'file': file_obj}
            data = {'match_type': mode, 'sport_type': sport_type}
...
if __name__ == "__main__":
    ...
    print(f"{Colors.BOLD}NeuralAce Auto Simulator v2.1{Colors.ENDC}")

    if not check_server():
        sys.exit(1)

    sport = input("Select sport (b=badminton, t=table_tennis) [b]: ").strip().lower()
    sport_type = "table_tennis" if sport == "t" else "badminton"
    video = input(f"Video path [{TEST_VIDEO_PATH}]: ").strip() or TEST_VIDEO_PATH
    print(f"Sport: {sport_type} | Video: {video}")

    while True:
        session_id_to_feedback = simulate_rally(video, mode='singles', sport_type=sport_type)
